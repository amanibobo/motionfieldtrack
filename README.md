...
The name MotionField is an ironic play on The Ideal Gas Law: PV=nRT - There's nothing ideal about the chaotic movement of real-life objects.
...
To directrly install MotionField to your computer choose the appropriate file for your operating system from [this folder](https://mega.nz/folder/ip1HQLLT#3_Qnl3GPnD9Ek6tTnrqAxw).

- **[MacOS:](https://mega.nz/file/e590WI6K#bjzjBUwIyd9jlPEXqiQZQ5MokuQTK9qk7CX9CxIAOMI)** After unzipping the ``MotionField for MacOS.zip``` file, you can double click ``MotionField.app``` to open. If an *"Unidentified developer"* error message pops up, right click the ``MotionField.app``` and choose open. The message should reapear with an ```open``` button this time.
